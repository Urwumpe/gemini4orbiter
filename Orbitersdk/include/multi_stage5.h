...
void Multi_Stage::clbkTimestep (double simt){
	this->Timestep(simt);
}

void Multi_Stage::clbkDrawHUD (int mode, const HUDPAINTSPEC *hps, HDC hDC){
}
...
bool Multi_Stage::clbkPanelMouseEvent (int id, int event, int mx, int my){
	return this->PanelMouseEvent(id,event,mx,my);
}

bool Multi_Stage::clbkPanelRedrawEvent (int id, int event, SURFHANDLE surf){
	return this->PanelRedrawEvent(id,event,surf);
}
...
bool Multi_Stage::clbkVCMouseEvent (int id, int event){
	return false;
}
